...
import { getCountryCode } from "./getCountry.ts";
...
const countriesFile = await Deno.open(countryDbPath, { read: true });
...
router
  ...
  .get("/api/country/:ip", async (context) => {
    if (context?.params?.ip && isIpv4Regex.test(context?.params?.ip)) {
      context.response.body = JSON.stringify(await getCountryCode(
        context?.params?.ip,
        countriesFile,
      ));
    } else {
